...
function asyncRouteLoader(source) {
  ...
  var resourcePath = this && this.resourcePath;
  ...
  recast.visit(ast, {
    visitCallExpression: function(path) {
      if (path.node.callee.type === 'MemberExpression' &&
        ...
        // Rewrite to require('createAsyncRouteHandler')(require('bundle!PATH_HERE'))
        // TODO: this is a bit cheesy -- it should be built with recast builders.
        var newAst = recast.parse(
          'require(' + JSON.stringify(require.resolve('./createAsyncRouteHandler')) +
            ')(require(' + JSON.stringify(
              require.resolve('bundle-loader') + '?lazy!' +
                path.node.arguments[1].properties[foundPropertyIndex].value.value
            ) + '))'
        );
