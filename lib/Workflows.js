...
var webpackRequire = require('./webpackRequire');
...
var Workflows = {
  ...
  serve: function(argv) {
    argv._[0] = argv._[0] || '.';
    var packageRoot = path.resolve(argv._[0]);
  ...
  validate: function(argv) {
    ...
    if (!fs.existsSync(argv._[0])) {
      console.error(argv._[0] + ' does not exist.');
      process.exit(1);
    }
    StandardWebpack.validate(require(path.resolve(argv._[0])));
  },
  ...
  static: function(argv) {
    ...
    StandardWebpack.shim(); // todo: remove
    ...
    var baseConfig = {
      ...
      module: {
        loaders: [
          {
            test: /\.jsx$/,
            loader: require.resolve('./stripAsyncRoutes') + '!' + require.resolve('babel-loader') + '?stage=1',
            include: packageRoot,
          },
          {
            test: /\.css$/,
    ...
    StandardWebpack.validate(baseConfig);

    webpackRequire(baseConfig, reactEntrypointFilename, function(err, mod) {
      var routes = mod();

      Router.run(routes, function(component) {
        var config = assign({}, baseConfig);
        config.resolve.alias.REACT_ROUTES = reactEntrypoint;
        ...
        config.module.loaders[0].loader = require.resolve('babel-loader') + '?stage=1';
