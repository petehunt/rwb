...
function configureHtml(html, config) {
  var newHtml = html ? String(html) : '';

  if (typeof config === 'object') {
    var title = config.title;
    var head = (config.head || []).join('\n    ');

    newHtml = newHtml.split('<!-- RWB TITLE -->').join(title);
    newHtml = newHtml.split('<!-- RWB HEAD -->').join(head);
  }
...
var Workflows = {
  ...
  serve: function (args) {
    ...
    var apiProxyConfig = packageJsonData.react.api;
