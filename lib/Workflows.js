    ...
    Router.run(routes, function(component) {
      ...
      webpack(config).run(function(err, stats) {
        ...
        stats.toJson().chunks.forEach(function(chunk) {
          ...
          } else if (chunk.names.indexOf('path:/') > -1) {
            invariant(chunk.files.length === 1, 'Too many chunks for root path');
            invariant(chunk.files[0].slice(-3) === '.js', 'Entry chunk was not .js: %s', chunk.files[0]);
            invariant(chunk.initial, 'Root chunk was not an initial chunk');
            scripts.push(chunk.files[0]);
          }
        });
        fs.writeFileSync(
          indexPath,
