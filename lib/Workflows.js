...
var Workflows = {
  init: function(args) {
    ...
    packageJsonData.dependencies['react-dom'] = (
      packageJsonData.dependencies['react-dom'] || myPackageJson.dependencies['react-dom']
    );
  ...
  static: function(args) {
    ...
    webpackRequire(config, require.resolve(reactEntrypoint), function(err, factory) {
      ...
      var routes = factory();
      var markup = ReactDOMServer.renderToString(React.createElement(Router.Router, {history: Router.createMemoryHistory('/')}, routes));
      fs.writeFileSync(
        indexPath,
        fs.readFileSync(
          path.join(__dirname, 'index.html'),
